...
function PelayananPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        tanggalProses: form.tanggalProses || null,
  ...
  const handleExportPdf = () => {
    exportHtmlToPdf({
      title: "Laporan Data Pelayanan",
      subtitle: "Rekap tindak lanjut pelayanan pengaduan warga pada sistem SIPANDU.",
      summary: [
        { label: "Total Pelayanan", value: pelayanan.length },
        { label: "Diproses", value: pelayanan.filter((item) => getStatus(item) === "DIPROSES").length },
        { label: "Selesai", value: pelayanan.filter((item) => getStatus(item) === "SELESAI").length },
        { label: "Ditolak", value: pelayanan.filter((item) => getStatus(item) === "DITOLAK").length },
      ],
      columns: ["No", "Nama Warga", "Pengaduan", "Keterangan", "Tanggal Proses", "Status"],
      rows: pelayanan.map((item, index) => [
        index + 1,
        getNamaWarga(item),
        getJudulPengaduan(item),
        item?.keterangan || "-",
        formatTanggal(item?.tanggalProses || item?.tanggal_proses || item?.createdAt || item?.created_at),
        formatStatus(getStatus(item)),
      ]),
      footer: "Dokumen ini dibuat otomatis dari menu Data Pelayanan SIPANDU.",
    });
  };

  const handleExportDetail = (item) => {
    exportHtmlToPdf({
      title: "Detail Pelayanan",
      subtitle: "Ringkasan tindak lanjut pelayanan pengaduan warga.",
      details: [
        { label: "ID Pelayanan", value: getIdPelayanan(item) || "-" },
        { label: "Nama Warga", value: getNamaWarga(item) },
        { label: "Judul Pengaduan", value: getJudulPengaduan(item) },
        { label: "Admin", value: item?.namaAdmin || "Admin Kecamatan" },
        { label: "Tanggal Proses", value: formatTanggal(item?.tanggalProses || item?.tanggal_proses || item?.createdAt || item?.created_at) },
        { label: "Status", value: formatStatus(getStatus(item)) },
        { label: "Keterangan", value: item?.keterangan || "-" },
      ],
      footer: "Detail ini dicetak dari sistem SIPANDU dan digunakan sebagai arsip tindak lanjut pelayanan.",
    });
  };
                          ...
                          <td>
                            <div className="action-buttons">
                              <button
                                ...
                                onClick={() => handleExportDetail(item)}
                              >
                                Detail PDF
                              </button>
